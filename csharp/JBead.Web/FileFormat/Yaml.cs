...
public static class Yaml
{
    ...
    private static object? ParseNode(string[] lines, ref int pos, int indent)
    {
        SkipBlank(lines, ref pos);
        if (pos >= lines.Length) return null;
        var line = StripComment(lines[pos]);
        var myIndent = IndentOf(line);
        if (myIndent < indent) return null;
        var content = line.Substring(myIndent);
    ...
    private static Dictionary<string, object?> ParseBlockMapping(string[] lines, ref int pos, int indent)
    {
        ...
        while (pos < lines.Length)
        {
            ...
            var content = line.Substring(lineIndent);
            if (content.StartsWith("-")) return map; // sequence begins here — caller handles

            var colon = FindTopLevelColon(content);
            if (colon < 0) { pos++; continue; }

            var key = UnquoteKey(content.Substring(0, colon));
            var rest = content.Substring(colon + 1).TrimStart();
            pos++;
            ...
            var next = StripComment(lines[look]);
            var nextIndent = IndentOf(next);
            if (nextIndent <= indent) { map[key] = null; continue; }
            var nextContent = next.Substring(nextIndent);
    ...
    {
        var i = 0;
        var value = ParseFlowValue(s, ref i);
        return value;
    }
    ...
    private static object? ParseFlowScalar(string s, ref int i)
    {
        ...
        if (s[i] == '"' || s[i] == '\'') return ReadQuoted(s, ref i);
        var start = i;
    ...
    private static object? ParseScalar(string s)
    {
        ...
        if (long.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out var iv))
    ...
    {
        while (pos < lines.Length && string.IsNullOrWhiteSpace(StripComment(lines[pos]))) pos++;
    }
    ...
    private static string StripComment(string line)
    {
        ...
        for (var i = 0; i < line.Length; i++)
        {
            ...
            else if (inQuote && ch == quote && (i == 0 || line[i - 1] != '\\')) inQuote = false;
            else if (ch == '#' && !inQuote) return line.Substring(0, i);
        }
        return line;
    }
    ...
    private static int FindTopLevelColon(string s)
    {
        ...
        for (var i = 0; i < s.Length; i++)
        {
            var ch = s[i];
            if (!inQuote && (ch == '"' || ch == '\'')) { inQuote = true; quote = ch; }
            else if (inQuote && ch == quote && (i == 0 || s[i - 1] != '\\')) inQuote = false;
            else if (!inQuote)
            {
                if (ch == '{' || ch == '[') depth++;
                else if (ch == '}' || ch == ']') depth--;
    ...
    private static void WriteNode(StringBuilder sb, object? node, int indent, bool atLineStart)
    {
        ...
        {
            case null:
                if (atLineStart) Indent(sb, indent);
                sb.Append("null");
                break;
            case string s:
                if (atLineStart) Indent(sb, indent);
    ...
    private static void WriteMapping(StringBuilder sb, IDictionary<string, object?> map, int indent, bool atLineStart)
    {
        ...
        {
            if (!first || !atLineStart) sb.Append('\n');
            first = false;
            Indent(sb, indent);
    ...
    {
        var json = (fmt & YamlFormat.Json) != 0;
        var spaced = (fmt & YamlFormat.Spaced) != 0;
    ...
    private static void WriteFlowValue(StringBuilder sb, object? node, bool json, bool spaced)
    {
        switch (node)
        {
            ...
            case double d: sb.Append(d.ToString("G", CultureInfo.InvariantCulture)); return;
            case string s:
                sb.Append(json ? Quote(s) : FormatScalar(s));
                return;
            case IDictionary<string, object?> map:
                sb.Append('{');
                if (spaced && map.Count > 0) sb.Append(' ');
                {
                    var first = true;
                    foreach (var kv in map)
                    {
                        if (!first) sb.Append(spaced ? ", " : ",");
                        first = false;
            ...
            case IEnumerable<object?> seq:
                {
                    var list = seq as IList<object?> ?? new List<object?>(seq);
                    sb.Append('[');
                    if (spaced && list.Count > 0) sb.Append(' ');
                    for (var i = 0; i < list.Count; i++)
                    {
                        if (i > 0) sb.Append(spaced ? ", " : ",");
    ...
    {
        if (NeedsQuoting(key)) return Quote(key);
        return key;
    }
    ...
    private static bool NeedsQuoting(string s)
    {
        ...
        if (s == "null" || s == "true" || s == "false" || s == "~") return true;
        if (int.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out _)) return true;
